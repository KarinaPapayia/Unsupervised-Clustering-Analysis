...
from lib.utils import target_distribution, cluster_accuracy
...
def train(dataset: torch.utils.data.Dataset,
    ...
    #Arguments:
        dataset: instance of Dataset to use for training
        model: instance of DEC model to train
        epochs: number of training epochs
        batch_size: size of the batch to train with
        optimizer: instance of optimizer to use
        stopping_delta: label delta as a proportion to use for stopping, None to disable, default None
        collate_fn: function to merge a list of samples into mini-batch
        cuda: whether to use CUDA, defaults to True
        sampler: optional sampler to use in the DataLoader, defaults to None
        silent: set to True to prevent printing out summary statistics, defaults to False
        update_freq: frequency of batches with which to update counter, None disables, default 10
        evaluate_batch_size: batch size for evaluation stage, default 1024
        update_callback: optional function of accuracy and loss to update, default None
        epoch_callback: optional function of epoch and model, default None
     #Return: None
    """
